...
module.exports = class IGCAnalyzer {
  ...
  parse(extracalculations = false, filter = false) {
    ...
    if (extracalculations) {
      ...
      const steps = 2; // <- make this dynamic!
      ...
      for (const f of this.fixes) {
        ...
        if (f.status === 'g' && f.vario >= 0) {
          ...
          } else if (f.vario > this.currentUpwind['max-climb']) {
            this.currentUpwind['max-climb'] = f.vario;
          }
        ...
        if (f.status == 'g' && f.vario <= this.config['min-sinkarea-vario']) {
          ...
          } else if (f.vario < this.currentSink['max-sink']) {
            this.currentSink['max-sink'] = f.vario;
          }
      ...
      for (const k in this.thermals) {
        const v = this.thermals[k];
      ...
      for (const k in this.upwinds) {
        const v = this.upwinds[k];
        if (typeof v.merged !== 'undefined') {
      ...
      for (const k in this.sinks) {
        const v = this.sinks[k];
        if (typeof v.merged !== 'undefined') {
          delete (this.sinks[k]);
        }
      }
      ...
      for (const t of this.thermals) {
        ...
        if (t.duration > 60 && t.heightgain > 100) {
          const speed = (t.distance / 1000) / (t.duration / 60 / 60) * this.config['wind-speed-correction-factor'];
  ...
  parseARecord(data) {
    const manufacturer = data.substr(0, 3);
    const uid = data.substr(3, 3);
  ...
  parseHRecord(data) {
    const datasource = data.substr(0, 1);
    const subtype = data.substr(1, 3);
    let title;
    let
      value;
  ...
  parseIRecord(data) {
    const cnt = Number(data.substr(0, 2));
    const start = Number(data.substr(2, 2));
    const end = Number(data.substr(4, 2));
    const code = data.substr(6, 3);
  ...
  parseBRecord(data, extracalculations, filter) {
    const time = data.substr(0, 6);
    const lat = data.substr(6, 8);
    const lng = data.substr(14, 9);
    const validity = data.substr(23, 1);
    const pressurealt = Number(data.substr(24, 5));
    const gpsalt = Number(data.substr(29, 5));
    ...
    const minutes = Number(time.substr(2, 2));
    const seconds = Number(time.substr(4, 2));

    const timestamp = seconds + (minutes * 60) + (hours * 60 * 60);
    ...
    if ((_.has(this, 'lastfix.time.t') && timestamp - this.lastfix.time.t >= this.minStepDuration) || !_.has(this, 'lastfix.time.t')) {
      ...
      for (const fadd of this.fixadditions) {
        const value = data.substr(fadd.start - 2, fadd.end - fadd.start + 1);
      ...
      if (extracalculations && fixIsOk) {
        ...
        const testfixes = [fix];
        for (let i = this.fixes.length - 1; i >= 0; i--) {
          const duration = fix.time.t - this.fixes[i].time.t;
          testfixes.push(this.fixes[i]);
          if (duration >= this.config['min-turning-detection-duration']) break;
        }

        for (const f of testfixes) {
          if (f.altitude > maxheight) maxheight = f.altitude;
          if (f.altitude < minheight) minheight = f.altitude;
          bearings.push(f.bearing);
        }

        const bearingDeviation = ss.standardDeviation(bearings);
        const maxBearingDifference = this.bearing_difference(bearings, this.config['min-turning-bearing-difference']);
  ...
  parseBRecordLatitude(str) {
    const degree = Number(str.substr(0, 2));
  ...
  parseBRecordLongitude(str) {
    const degree = Number(str.substr(0, 3));
  ...
  bearing_difference(samples, maxdiff = 180) {
    ...
    outer: for (const s1 of uniqueSamples) {
      for (const s2 of uniqueSamples) {
        const diff = this.abs_bearing_difference(s1, s2);
        if (diff > max) {
          max = diff;
